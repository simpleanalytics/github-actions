...
For `change: needs review`, the workflow ensures the Simple Analytics PR template is used and that a linked tracking issue exists. It first tries to create the issue in `simpleanalytics/dashboard`; if that is not accessible, it falls back to the current repository.
